...
namespace ivm {
	...
	constexpr auto EXIT() -> ivm_rgstr {

		return ivm_rgstr(IVM_EXIT ^ ivm_seed);
	}

	template <typename T = ivm_default>
	constexpr auto MOV(ivm_rgstr rgstr1, ivm_rgstr rgstr2 = 0) -> ivm_instr {

		return build_instr<T>(rgstr1, rgstr2, IVM_MOV);
	}

	template <typename T = ivm_default>
	constexpr auto ADD(ivm_rgstr rgstr1, ivm_rgstr rgstr2 = 0) -> ivm_instr {

		return build_instr<T>(rgstr1, rgstr2, IVM_ADD);
	}

	template <typename T = ivm_default>
	constexpr auto SUB(ivm_rgstr rgstr1, ivm_rgstr rgstr2 = 0) -> ivm_instr {

		return build_instr<T>(rgstr1, rgstr2, IVM_SUB);
	}

	template <typename T = ivm_default>
	constexpr auto XOR(ivm_rgstr rgstr1, ivm_rgstr rgstr2 = 0) -> ivm_instr {

		return build_instr<T>(rgstr1, rgstr2, IVM_XOR);
	}

	template <typename T = ivm_default>
	constexpr auto LEA(ivm_rgstr rgstr1, ivm_rgstr rgstr2) -> ivm_instr {

		return build_instr<T>(rgstr1, rgstr2, IVM_LEA);
	}

	template <typename T = ivm_default>
	constexpr auto CMP(ivm_rgstr rgstr1, ivm_rgstr rgstr2 = 0) -> ivm_instr {

		return build_instr<T>(rgstr1, rgstr2, IVM_CMP);
	}

	template <typename T = ivm_default>
	constexpr auto JNE(ivm_rgstr rgstr1 = 0) -> ivm_instr {

		return build_instr<T>(rgstr1, rgstr1, IVM_JNE);
	}

	template <typename T = ivm_default>
	constexpr auto CNA(ivm_rgstr rgstr1 = 0) -> ivm_instr {

		return build_instr<T>(rgstr1, rgstr1, IVM_CNA);
	}
	...
	auto ivm(const std::vector<ivm_instr> &prgm, bool dbg = true) -> void {
		...
		while(prgm_counter < prgm.size()) {
			...
			const auto immv = ((instr >> 7) & IVM_IMMV);
			const auto size = ((instr >> 4) & 0x7) + 1;

			const auto deref = (oprnd >> 6) & 0x3;

			const auto rgstr1 = (oprnd & 0x7);
			const auto rgstr2 = (oprnd >> 3) & 0x7;
